...
.if ${_LANGCXX_SHARED_FORMAT} == elf
_LANGCXX_SHARED_SUFFIX=	so
.elif ${_LANGCXX_SHARED_FORMAT} == mach-o
_LANGCXX_SHARED_SUFFIX=	dylib
.else
.error ${_LANGCXX_SHARED_FORMAT}: Unsupported shared object format.
.endif
...
LIB+=			lib${LIBRARY}.${_LANGCXX_SHARED_SUFFIX}.${LIBVERSION}
CLEANFILES+=		lib${LIBRARY}.${_LANGCXX_SHARED_SUFFIX}.${LIBVERSION}
.else
LIB+=			lib${LIBRARY}.${_LANGCXX_SHARED_SUFFIX}
CLEANFILES+=		lib${LIBRARY}.${_LANGCXX_SHARED_SUFFIX}
.endif
.endif
INCLUDE+=		${SRCS:M*.hpp}
CLEANFILES+=		${OBJS}
...
.if defined(LIBVERSION)
lib${LIBRARY}.${_LANGCXX_SHARED_SUFFIX}.${LIBVERSION}: ${OBJS}
.else
lib${LIBRARY}.${_LANGCXX_SHARED_SUFFIX}: ${OBJS}
.endif
